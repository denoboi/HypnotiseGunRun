	...
	public class HyperBoxToolGUI : MonoBehaviour
	{
		...
		private static void DrawLevelArea()
		{
			...
			if (SelectLevel != lastSelectLevel)
			{
				if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())
					EditorSceneManager.OpenScene(LevelData.Levels[SelectLevel].LevelProperties[LevelData.Levels[SelectLevel].LoadLevelID]);
			}
		...
		private static LevelData LevelData
		{
			get
			{
				if (levelData == null)
				{
					...
					levelData = AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(levelDatas[0]), typeof(LevelData)) as LevelData;
				}
				return levelData;
			}
		}
		...
		public static void PlayScene(int level)
		{
			...
			if (!EditorApplication.isPlaying)
			{
				...
				{
					if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())
						EditorSceneManager.OpenScene(LevelData.Levels[level].LevelProperties[LevelData.Levels[level].LoadLevelID]);
				}
			}
			else
			{
